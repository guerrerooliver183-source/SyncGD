...
class $modify(SRAccountLayer, AccountLayer) {
    ...
    void customHideLoadingUI() {
        ...
        if (m_fields->m_cancelMenu) m_fields->m_cancelMenu->setEnabled(false);
        if (m_fields->m_backupLabel) m_fields->m_backupLabel->setVisible(false);
        if (m_fields->m_cancelButton) m_fields->m_cancelButton->setVisible(false);
    }
